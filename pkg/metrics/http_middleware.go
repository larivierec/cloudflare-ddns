...
type HttpApiMetrics interface {
	...
	CollectProviderRequest(provider api.Interface)
}
...
func (h *httpApiMetrics) CollectProviderRequest(provider api.Interface) {
	h.providerRequests.WithLabelValues(provider.GetProviderName()).Inc()
}
